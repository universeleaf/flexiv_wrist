...
def realtime_preflight(executable: Path, cpu_affinity: int) -> list[str]:
    ...
    if os.geteuid() != 0 and soft_rtprio < 99:
        raise RuntimeError(
            ...
            "按 docs/RT_TORQUE_OSC_ZH.md 把 realtime 组改为 rtprio 99，"
            "完全注销并重新登录后再运行"
        )
...
def write_python_orientation_dry_run(
    ...
    if duration_s < 8.0 or not 0.0 < orientation_deg <= 25.0:
        raise ValueError("duration_s>=8 且 orientation_deg 在 (0,25] 内")
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", newline="", encoding="utf-8") as stream:
        writer = csv.writer(stream)
        writer.writerow(
