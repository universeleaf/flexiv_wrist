...
# Both projects historically call their Python package ``src``.  Extend this
# project's package search path so the tested pedal modules can coexist with
# the teleoperation math without copying either implementation.
import src as _combined_src  # noqa: E402

_combined_src.__path__.append(str(FREEDRIVE_ROOT / "src"))

from run_teleop import BridgeReader, ensure_robot_ready, wait_for_first_sample  # noqa: E402
from src.nine_dof_core import (  # noqa: E402
...
from src.teleop_core import (  # noqa: E402
    MappingConfig,
    RelativePoseMapper,
...
class WristBridge:
    def __init__(
        ...
        *,
        ...
        bridge = PROJECT_ROOT / "scripts" / "wrist_moteus_bridge.py"
        if not executable.is_file():
        ...
        try:
            for line in self._process.stdout:
                ...
                if message.startswith("WRIST_POSITION_WRAP"):
                    print(f"[wrist] {message}")
                    continue
                if message.startswith(("WRIST_POSITION_CAPTURE", "WRIST_POSITION_STATUS")):
                    print(f"[wrist] {message}")
                    continue
                ...
                if len(fields) != 9 or fields[0] != "WRIST_SAMPLE":
            ...
            while True:
                ...
                if enabled:
                    ...
                    if selected_mode is TeleopMode.ARM_7DOF:
                        ...
                        wrist_goal_q = result.q_target_rad
                        if selected_mode is TeleopMode.ARM_WRIST_9DOF:
