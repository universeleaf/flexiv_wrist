#!/usr/bin/env python3
"""Local browser UI for configuring and running the 9-DoF teleoperator."""
...
TELEOP_SCRIPT = PROJECT_ROOT / "scripts" / "run_9dof_teleop.py"
...
from src.config_ui import (  # noqa: E402
...
class TeleopProcessManager:
    ...
    def _read_process(self, process: subprocess.Popen[str]) -> None:
        ...
        try:
            ...
            self._append(f"[UI] Teleoperation process exited with code {return_code}")
    ...
    def start(self, config_path: Path) -> dict[str, Any]:
        with self._lock:
            ...
            self._append("[UI] Teleoperation started; waiting for wrist and Flexiv")
...
<head>
  ...
  <title>Flexiv 9-DoF Teleoperation Console</title>
  <style>
...
<body>
<header>
  <div><h1>Flexiv 9-DoF Teleoperation Console</h1><div class="small">Localhost only · Foot pedals and UI mode buttons work together</div></div>
    ...
    <section class="panel">
      ...
      <div class="panel-body">
        <div class="control-grid">
          <button id="startBtn" class="primary">Save &amp; Start</button>
...
function sectionTitle(name) {
  const titles={robot:'Flexiv Arm',haptic:'omega.7 Haptic Device',pedal:'Foot Pedals',wrist:'Wrist Motors',
    wrist_geometry:'Wrist Geometry',wrist_payload:'Wrist Payload',flexiv_tool:'Flexiv Tool',allocation:'9-DoF Allocation',
    pivot_orientation_osc:'Mode 3 OSC',force_feedback:'Haptic Feedback',runtime:'Real-Motion Confirmation'};
...
async function startTeleop() {
  ...
  try {
    ...
    await saveConfig();
    action.textContent='Starting teleoperation process…';
    const state=await api('/api/teleop/start',{method:'POST',body:{}});
    updateStatus(state); action.textContent='Process started. Waiting for wrist and Flexiv…';
  } catch(error) {
...
function updateStatus(state) {
  ...
  document.querySelectorAll('.mode').forEach(button=>{button.disabled=!running;button.classList.toggle('active',state.mode===({1:'7dof',2:'9dof',3:'pivot_orientation'})[button.dataset.mode]);});
...
document.getElementById('startBtn').addEventListener('click',startTeleop);
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:  # noqa: N802
            ...
            try:
                ...
                elif self.path == "/api/teleop/start":
                    self._json(manager.start(store.path))
...
def main() -> int:
    ...
    print(f"Flexiv Teleoperation UI: {url}")
    print("Press Ctrl-C in this terminal to close the UI; active teleoperation will be stopped first.")
    if not args.no_browser:
        threading.Timer(0.4, lambda: webbrowser.open(url)).start()
    try:
        server.serve_forever(poll_interval=0.25)
    except KeyboardInterrupt:
        print("\nStopping teleoperation and the UI server...")
    finally:
        server.shutdown()
        server.server_close()
        manager.shutdown()
