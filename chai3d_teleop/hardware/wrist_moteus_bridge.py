...
Protocol on stdin (SI units):
  ...
  H <q1_rad> <q2_rad> <tau1_Nm> <tau2_Nm>
                            OSC target position + feed-forward torque
...
def _parse_command(line: str) -> tuple[str, list[float]]:
    ...
    expected = {"P": 2, "P8": 1, "T": 2, "H": 4, "Z": 0, "A": 0, "S": 0}
    if operation not in expected or len(fields) - 1 != expected[operation]:
        raise ValueError(
            "命令必须是 P q1 q2、P8 q1、T tau1 tau2、"
            "H q1 q2 tau1 tau2、Z、A 或 S"
        )
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "启动回零期间对 moteus 永久 kp 的临时缩放；省略时使用 "
            "--position-kp-scale"
        ),
    )
    parser.add_argument(
        ...
        default=None,
        help=(
            "启动回零期间对 moteus 永久 kd 的临时缩放；省略时使用 "
            "--position-kd-scale"
        ),
    )
...
def _validate_args(args: argparse.Namespace) -> None:
    if args.zero_position_kp_scale is None:
        args.zero_position_kp_scale = tuple(args.position_kp_scale)
    if args.zero_position_kd_scale is None:
        args.zero_position_kd_scale = tuple(args.position_kd_scale)
    ...
    positive_fields = (
        ...
        *args.zero_position_kp_scale,
        *args.zero_position_kd_scale,
        args.velocity_deg_s,
        args.accel_deg_s2,
        ...
        args.position_following_error_timeout_s,
        args.loop_hz,
    ...
    if any(value > 1.0 for value in gain_scales):
...
async def _run(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        samples = await _move_to_zero(
            ...
            kp_scale=args.zero_position_kp_scale,
            kd_scale=args.zero_position_kd_scale,
        ...
        print(
            ...
            "limit_deg={} reduction_ratio={} zero_position_kp_scale={} "
            "zero_position_kd_scale={} runtime_position_kp_scale={} "
            "runtime_position_kd_scale={} torque_control={}".format(
                ...
                ",".join(f"{value:g}" for value in args.zero_position_kp_scale),
                ",".join(f"{value:g}" for value in args.zero_position_kd_scale),
        ...
        while True:
            ...
            if watchdog_armed and operation in ("P", "P8", "T", "H") and age_ms > args.watchdog_ms:
            ...
            commanded_tau: list[float | None] = [None, None]
            soft_limit_active = [False, False]
            if operation == "I":
            ...
            elif operation in ("P", "H"):
                if last_applied_operation not in ("P", "H"):
                ...
                position_command = command[:2]
                feedforward_command = command[2:] if operation == "H" else [0.0, 0.0]
                for index, (controller, motor_id, q_target, requested_tau, zero, sign, limit, torque_limit, kp, kd) in enumerate(zip(
                    controllers,
                    args.ids,
                    position_command,
                    feedforward_command,
                    ...
                    requested_tau = float(np_clip(requested_tau, -torque_limit, torque_limit))
                    target_rev = zero + sign * q_target / TAU
                    commanded_q[index] = q_target
                    commanded_rev[index] = target_rev
                    commanded_tau[index] = requested_tau if operation == "H" else None
                    state = await _command_with_timeout(
                        controller.set_position(
                            position=target_rev,
                            velocity=0.0,
                            feedforward_torque=(
                                sign * requested_tau if operation == "H" else 0.0
                            ),
                ...
                for index, (controller, requested, zero, sign, limit, torque_limit) in enumerate(
                    ...
                    commanded_tau[index] = tau
                    state = await _command_with_timeout(
                        controller.set_position(
                            position=math.nan,
                            # NaN velocity and zero kp/kd make this an explicit
                            # pure feed-forward output-torque command.  A
                            # numeric zero velocity is unnecessary and can
                            # obscure diagnosis of an incorrectly retained
                            # velocity loop on older firmware.
                            velocity=math.nan,
            ...
            if operation in ("T", "H"):
                ...
                bad_modes = [
                    (motor_id, sample.mode, sample.fault)
                    for motor_id, sample in zip(args.ids, samples)
                    if sample.mode != expected_mode
                ]
                torque_mode_mismatch_cycles = (
                    torque_mode_mismatch_cycles + 1 if bad_modes else 0
                )
                if torque_mode_mismatch_cycles == 3:
                    details = ", ".join(
                        f"ID{motor_id} mode={mode} fault={fault}"
                        for motor_id, mode, fault in bad_modes
                    )
                    print(
                        "WRIST_TORQUE_WARNING moteus 暂未处于 mode=10，"
                        "继续重发力矩命令自动恢复：" + details,
                        file=sys.stderr,
                        flush=True,
                    )

                for index, (requested, values) in enumerate(
                    zip(commanded_tau, joint_states)
                ):
                    if requested is None:
                        torque_tracking_mismatch_cycles[index] = 0
                        continue
                    applied = values[2]
                    tolerance = max(0.15, 0.30 * abs(requested))
                    # In hybrid mode the measured torque also contains the
                    # drive's position-loop contribution, so it is not
                    # expected to equal the feed-forward term by itself.
                    mismatch = operation == "T" and (
                        abs(requested) >= 0.20
                        and abs(applied - requested) > tolerance
                    )
                    torque_tracking_mismatch_cycles[index] = (
                        torque_tracking_mismatch_cycles[index] + 1
                        if mismatch
                        else 0
                    )
                now = time.monotonic()
                if now >= next_torque_status:
                    details = " ".join(
                        "ID{}[requested={:+.3f}Nm applied={:+.3f}Nm "
                        "mode={} fault={} soft_limit={} tracking_mismatch_cycles={}]".format(
                            motor_id,
                        ...
                        for motor_id, requested, values, sample, soft_active, mismatch_cycles in zip(
                            args.ids,
                            commanded_tau,
                            joint_states,
                            samples,
                            soft_limit_active,
                            torque_tracking_mismatch_cycles,
                        )
                    )
                    print(
                        f"WRIST_TORQUE_STATUS operation={operation} {details}",
                        flush=True,
                    )
                    next_torque_status = now + 0.5
            following_violations: list[tuple[int, str]] = []
            following_now = time.monotonic()
            for index, (motor_id, values, target_q, target_rev, sample) in enumerate(zip(
                args.ids,
                joint_states,
                commanded_q,
                commanded_rev,
                ...
                if target_q is None:
                    following_violation_since[index] = None
                    continue
                error_deg = abs(math.degrees(values[0] - target_q))
                if error_deg > args.position_following_error_deg:
                    if following_violation_since[index] is None:
                        following_violation_since[index] = following_now
                    ...
                    following_violation_since[index] = None
            if following_violations and operation == "H":
                # Hybrid OSC uses a continuously changing filtered IK target.
                # A transient tracking error is diagnostic, not a reason to
                # tear down Flexiv RT control. Physical travel, drive faults,
                # and communication loss remain hard stops below/above.
                now = time.monotonic()
                if now >= next_position_status:
                    print(
                        "WRIST_TRACKING_WARNING "
                        + "; ".join(message for _, message in following_violations),
                        file=sys.stderr,
                        flush=True,
                    )
            elif following_violations:
                persistent = [
                    message
                    for index, message in following_violations
                    if following_violation_since[index] is not None
                    and following_now - following_violation_since[index]
                    >= args.position_following_error_timeout_s
                ]
                if persistent:
                    raise RuntimeError(
                        "腕部位置目标持续未被正确跟随；已 STOP（这不是行程限位）："
                        + "; ".join(persistent)
                    )
                if following_now >= next_following_warning:
                    print(
                        "WRIST_TRACKING_TRANSIENT "
                        + "; ".join(message for _, message in following_violations),
                        file=sys.stderr,
                        flush=True,
                    )
                    next_following_warning = following_now + 0.5
            limit_violations = [
            ...
            print(
                "WRIST_SAMPLE {} {} {} {} {} {} {} mode={} servo_mode={} fault={}".format(
                    time.monotonic_ns(),
                    *[f"{value:.9g}" for value in flattened],
                    operation,
                    ",".join(
                        str(-1 if sample.mode is None else sample.mode)
                        for sample in samples
                    ),
                    ",".join(
                        str(-1 if sample.fault is None else sample.fault)
                        for sample in samples
                    ),
                ),
                flush=True,
