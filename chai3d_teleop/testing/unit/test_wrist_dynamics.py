...
from controllers.nine_dof import WristGeometry
from controllers.wrist_dynamics import WristDynamics, WristInertialParameters
...
def test_composite_spatial_inertia_is_6x6_symmetric_and_configuration_dependent() -> None:
    model = _model()
    zero = model.composite_spatial_inertia(np.zeros(2))
    moved = model.composite_spatial_inertia(np.array([0.4, -0.7]))
    assert zero.shape == (6, 6)
    assert np.allclose(zero, zero.T, atol=1e-12)
    assert np.min(np.linalg.eigvalsh(zero)) > 0.0
    assert not np.allclose(zero, moved)
    expected_mass = (
        model.parameters.rigid_body_scale
        * (model.parameters.link1_mass_kg + model.parameters.link2_mass_kg)
    )
    assert np.allclose(zero[:3, :3], expected_mass * np.eye(3))


def test_composite_spatial_inertia_equals_sum_of_body_inertias() -> None:
    model = _model()
    q = np.array([-0.2, 0.8])
    body1, body2 = model.body_spatial_inertias(q)
    assert np.allclose(model.composite_spatial_inertia(q), body1 + body2)
