...
def _atomic_update(path: Path, updated: str) -> Path:
    # Keep one recoverable pre-zero snapshot instead of accumulating an
    # unbounded directory of timestamped runtime artifacts.
    backup = path.with_name(f"{path.stem}.before-zero.toml")
