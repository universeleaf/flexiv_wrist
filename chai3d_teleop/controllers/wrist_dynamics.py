...
from .nine_dof import WristGeometry, axis_angle_rotation
...
class WristDynamics:
    ...
    def _body_poses(
        ...
        """Return both body CoMs and rotations in the Flexiv flange frame."""
        ...
        c2 = p2 + r12 @ p.link2_com_after_joint2_m
        return c1, c2, r1, r12
    ...
    def _body_jacobians(
        ...
        q = _vector(q_rad, 2, "q_rad")
        c1, c2, r1, r12 = self._body_poses(q)
        p1 = self.geometry.joint1_origin_flange_m
        p2 = p1 + r1 @ self.geometry.joint2_offset_after_joint1_m
        ...
        return jv1, jw1, jv2, jw2, r1, r12

    @staticmethod
    def _spatial_inertia_at_origin(
        mass_kg: float, com_m: np.ndarray, inertia_com: np.ndarray
    ) -> np.ndarray:
        """Build a 6x6 inertia for twist order [linear; angular]."""
        cross = np.array(
            [
                [0.0, -com_m[2], com_m[1]],
                [com_m[2], 0.0, -com_m[0]],
    ...
    def body_spatial_inertias(
        self, q_rad: np.ndarray
    ) -> tuple[np.ndarray, np.ndarray]:
        """Return both moving bodies' 6x6 inertias at the flange origin.

        Reflected motor/reducer inertia is a joint-coordinate quantity and is
        deliberately excluded here. It remains present in :meth:`mass_matrix`.
        """
        p = self.parameters
        c1, c2, r1, r12 = self._body_poses(q_rad)
        inertia1 = r1 @ p.link1_inertia_com_kg_m2 @ r1.T
        inertia2 = r12 @ p.link2_inertia_com_kg_m2 @ r12.T
        scale = p.rigid_body_scale
        return (
            scale
            * self._spatial_inertia_at_origin(p.link1_mass_kg, c1, inertia1),
            scale
            * self._spatial_inertia_at_origin(p.link2_mass_kg, c2, inertia2),
        )

    def composite_spatial_inertia(self, q_rad: np.ndarray) -> np.ndarray:
        """Return the articulated wrist's frozen-shape 6x6 flange inertia."""
        link1, link2 = self.body_spatial_inertias(q_rad)
        result = link1 + link2
        if np.linalg.eigvalsh(result)[0] <= 1e-10:
            raise ValueError("composite wrist spatial inertia is not positive definite")
        return result
