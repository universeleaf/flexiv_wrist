...
inline Eigen::Matrix3d SpinRotationFromPhase(
    const Eigen::Matrix3d& initial_rotation, double phase,
    double tilt_amplitude_rad)
{
    // q8-like pitch makes the spatial orientation change visible while the
    // q9-like -Z rotation completes one full 0->360 degree turn during the
    // first half-cycle and returns 360->0 during the second half-cycle.  The
    // reversal is necessary because joint 9 has a finite +/-360 degree range.
    const double pitch = tilt_amplitude_rad * std::sin(phase);
    const double spin = kPi * (1.0 - std::cos(phase));
    const Eigen::AngleAxisd pitch_y(pitch, Eigen::Vector3d::UnitY());
    const Eigen::AngleAxisd spin_minus_z(spin, -Eigen::Vector3d::UnitZ());
    return initial_rotation * pitch_y.toRotationMatrix()
           * spin_minus_z.toRotationMatrix();
}

inline Eigen::Vector3d SpinAngularVelocityPerPhase(
    const Eigen::Matrix3d& initial_rotation, double phase,
    double tilt_amplitude_rad)
{
    constexpr double epsilon = 1e-5;
    const Eigen::Matrix3d minus = SpinRotationFromPhase(
        initial_rotation, phase - epsilon, tilt_amplitude_rad);
    const Eigen::Matrix3d plus = SpinRotationFromPhase(
        initial_rotation, phase + epsilon, tilt_amplitude_rad);
    return RotationError(minus, plus) / (2.0 * epsilon);
}

inline LoopTarget SpinTargetFromPhase(const Eigen::Vector3d& fixed_position,
    const Eigen::Matrix3d& initial_rotation, double phase, double phase_rate,
    double phase_acceleration, double tilt_amplitude_rad)
{
    const Eigen::Matrix3d rotation
        = SpinRotationFromPhase(initial_rotation, phase, tilt_amplitude_rad);
    const Eigen::Vector3d omega_per_phase = SpinAngularVelocityPerPhase(
        initial_rotation, phase, tilt_amplitude_rad);
    constexpr double epsilon = 1e-5;
    const Eigen::Vector3d domega_dphase
        = (SpinAngularVelocityPerPhase(initial_rotation, phase + epsilon,
               tilt_amplitude_rad)
              - SpinAngularVelocityPerPhase(initial_rotation, phase - epsilon,
                  tilt_amplitude_rad))
          / (2.0 * epsilon);
...
inline LoopTarget ContinuousSpinTarget(const Eigen::Vector3d& fixed_position,
    const Eigen::Matrix3d& initial_rotation, double elapsed_s,
    double frequency_hz, double tilt_amplitude_rad)
{
    if (frequency_hz <= 0.0) {
        throw std::invalid_argument("spin frequency must be positive");
    }
    const double omega = 2.0 * kPi * frequency_hz;
    const double period_s = 1.0 / frequency_hz;
    const double ramp_s = std::min(2.0, 0.25 * period_s);
    if (elapsed_s < ramp_s) {
        ...
        const double integrated_speed
            = 2.5 * std::pow(u, 4) - 3.0 * std::pow(u, 5)
              + std::pow(u, 6);
        return SpinTargetFromPhase(fixed_position, initial_rotation,
            omega * ramp_s * integrated_speed, omega * speed_scale,
            omega * ds_du / ramp_s, tilt_amplitude_rad);
    }
    const double phase
        = omega * (elapsed_s - ramp_s) + 0.5 * omega * ramp_s;
    return SpinTargetFromPhase(fixed_position, initial_rotation, phase, omega,
        0.0, tilt_amplitude_rad);
}
...
inline void WriteSpinDryRunCsv(const std::string& path, double duration_s,
    double tilt_amplitude_rad)
{
    std::ofstream output(path);
    if (!output) throw std::runtime_error("cannot open spin dry-run CSV");
    output << "time_s,x_m,y_m,z_m,joint8_like_tilt_deg,"
    ...
    for (int index = 0; index <= steps; ++index) {
        ...
        const double pitch = tilt_amplitude_rad * std::sin(phase);
        const double spin = kPi * (1.0 - std::cos(phase));
        output << std::setprecision(10) << time_s << ",0,0,0,"
               << pitch * 180.0 / kPi << ',' << spin * 180.0 / kPi << '\n';
    }
}
